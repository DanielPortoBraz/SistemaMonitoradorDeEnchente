...
void ssd1306_levels(ssd1306_t *ssd, uint8_t level, uint8_t bar_x, uint8_t bar_y){

  
  uint8_t limit = 40 * level / 100;
  ...
  for (uint8_t i = 0; i < 6; i++){
    ...
    for (uint8_t j = 0; j < 38; j++){
      ...
      if(j < limit)
        ssd1306_pixel(ssd, i + bar_x, bar_y - j, true);
      
      else
        ssd1306_pixel(ssd, i + bar_x, bar_y - j, false);
    }
  }
